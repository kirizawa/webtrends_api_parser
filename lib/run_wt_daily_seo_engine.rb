...
month = ARGV[1]
wt_profile = ARGV[2]
daily_datfile = "../dat/daily_seo_engine_traffic_" + region + ".csv"
...
for i in 1..num_days
	...
	wt_query = "profiles/" + wt_profile + "/reports/erTHBg4DNj5/?totals=all&period=" + month + "d" + x + "&search=" + filter + "&measures=Visits-0&format=xml"
	...
	# -------------------------------------------------------------------------
	# Search Engine (organic):  This will parse xml and create csv file
	# -------------------------------------------------------------------------
	...
	doc.xpath('//list[@name="data"]/DataRow/list[@name="SubRows"]/DataRow[@name="' + search_engine + '"]').each do |data_row|
		...
		seo_engine_daily_traffic(csv, date, search_engine, visits)
